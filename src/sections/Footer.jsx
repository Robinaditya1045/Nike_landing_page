      ...
      <div className="flex justify-between items-start gap-20 flex-wrap max-lg:flex-col ">
        ...
        <div className="flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap">
              {footerLinks.map((section) => (
                <div key={section}>
                  <h4 className="text-white font-montserrat text-2xl leading-noramal mb-6">{section.title}</h4>
                  <ul>
                    {section.links.map((link) => (
                      <li key={link.name} className="text-white-400 font-montserrat text-base leading-noraml mt-3 hover:text-slate-gray cursor-pointer">
                        <a>{link.name}</a> 
                      </li>
                    ))}
                  </ul>
